...
async fn main() -> Result<()> {
    ...
    doc.paragraph("This tutorial mirrors the [ClickHouse NYC taxi tutorial](https://clickhouse.com/docs/tutorial), but shows the Diesel code next to the ClickHouse SQL. It uses the native async, HTTP-backed `AsyncClickHouseConnection` provided by this crate, so queries are driven with `diesel_async`'s `RunQueryDsl` and `.await`.");
    ...
    doc.paragraph("ClickHouse SQL clients connect to the server directly. In Diesel, create an `AsyncClickHouseConnection` from a URL or explicit options:");
    doc.rust(CONNECT_RUST);
    doc.paragraph("`AsyncClickHouseConnection::establish(\"http://user:password@host:8123/database\").await` is equivalent for URL-first configuration. The executable tutorial reads `CLICKHOUSE_URL` and uses `ClickHouseConnectionOptions::from_url(...)` so it can run against the disposable container created by `just tutorial` or against your own ClickHouse server.");
    ...
    diesel::sql_query(&create_trips_sql)
        .execute(&mut conn)
        .await?;
    ...
    let existing: i64 = trips::table.select(count_star()).first(&mut conn).await?;
    if existing == 0 {
        conn.batch_execute(INSERT_TRIPS_SQL).await?;
    }
    let row_count: i64 = trips::table.select(count_star()).first(&mut conn).await?;
    ...
    doc.rust("conn.batch_execute(\"DROP DICTIONARY IF EXISTS taxi_zone_dictionary\").await?;\nconn.batch_execute(CREATE_DICTIONARY_SQL).await?;");
    ...
    doc.rust(JOIN_RUST);
    let joined: Vec<BoroughTotal> = diesel::sql_query(JOIN_SQL).load(&mut conn).await?;
    ...
    doc.unordered_list(&[
        ...
        "Use `AsyncClickHouseConnection` for ordinary async `.load`, `.first`, `.execute`, and `.batch_execute` calls.",
...
diesel::sql_query(to_sql(&ddl)?).execute(&mut conn).await?;"#;

const INSERT_TRIPS_RUST: &str = r#"use diesel_async::{RunQueryDsl, SimpleAsyncConnection};
