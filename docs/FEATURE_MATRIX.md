...
| ✅ 🧪 | Single-row insert | `insert_into(t).values((c.eq(v), ...)).execute(&mut conn)` | Tuple and `#[derive(Insertable)]` (with `#[diesel(treat_none_as_default_value = false)]`)<br>single-row inserts render and execute; `execute` returns the written-row count (`1`). |
