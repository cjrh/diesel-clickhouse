...
- Idiomatic single-row inserts through the connection — `insert_into(t).values((col.eq(v), ...))` and `#[derive(Insertable)]` structs (with `#[diesel(treat_none_as_default_value = false)]`); multi-row batch inserts are not expressible through Diesel on a third-party backend, so high-throughput ingestion uses the `clickhouse` client's RowBinary inserter (see `docs/USAGE.md`)
