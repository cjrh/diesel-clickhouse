...
`docs/COOKBOOK.md` is a set of copyable recipes: each shows a piece of ClickHouse SQL next to the equivalent Diesel query. It is generated by an executable that runs **both** forms against a live server and asserts they return identical rows, so every recipe is verified, not just illustrative:
