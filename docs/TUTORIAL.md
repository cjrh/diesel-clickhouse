...
This tutorial mirrors the [ClickHouse NYC taxi tutorial](https://clickhouse.com/docs/tutorial), but shows the Diesel code next to the ClickHouse SQL. It uses the native async, HTTP-backed `AsyncClickHouseConnection` provided by this crate, so queries are driven with `diesel_async`'s `RunQueryDsl` and `.await`.
...
ClickHouse SQL clients connect to the server directly. In Diesel, create an `AsyncClickHouseConnection` from a URL or explicit options:
...
```rust,ignore
conn.batch_execute("DROP DICTIONARY IF EXISTS taxi_zone_dictionary").await?;
